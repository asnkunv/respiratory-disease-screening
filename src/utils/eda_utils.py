...
Reusable EDA utilities for respiratory disease screening datasets
(COUGHVID_v3, TB_screen, West China).
...
- Plotting functions return the matplotlib Figure object instead of calling
  plt.show(), so you can save them to disk in a pipeline or display them
...
def missingness_report(
    ...
    if plot and report["n_missing"].sum() > 0:
        ...
        plt.close(fig)  # prevent Jupyter's inline backend from auto-displaying it
...
def feature_label_correlation(
    ...
    *,
    ...
    1. Label already lives in features_df (e.g. a features CSV that also
       has a 'status' column). Just pass label_col, leave labels_df/key as
       None, no merge happens.
    2. Label lives in a separate table (e.g. COUGHVID's expert labels
       table keyed by 'uuid'). Pass labels_df and key, a merge happens
       first.
 
    Returns a DataFrame with columns: feature, f_score, p_value, sorted by
    f_score descending.
    """
    from sklearn.feature_selection import f_classif
 
    if labels_df is not None and key is not None:
        merged = features_df.merge(labels_df[[key, label_col]], on=key, how="inner").dropna(subset=[label_col])
        exclude_cols = {key, label_col}
    else:
        merged = features_df.dropna(subset=[label_col])
